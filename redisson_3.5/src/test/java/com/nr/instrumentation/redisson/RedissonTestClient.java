...
//	}
	...
	private static <V> V sync(Publisher<V> obs) {
		Promise<V> promise;
		if(Promise.class.isAssignableFrom(obs.getClass())) {
			promise = (Promise<V>)obs;
		} else {
			promise = Streams.wrap(obs).next();
		}
		V value = promise.poll();
		if(promise.isError()) {
			Error error = new Error(promise.reason());
